...
interface IMiscellaneousRequestParams {
	...
	headerData?: Record<string, any>; //for other header data
}
...
const commonAxios = (config: IAxiosParams): Promise<any> => {
	const { method, url, data, contentType = 'application/json', isPublic = false, headerData = {} } = config;
	let headers: any = {
	...
	if (!isPublic) {
		const token = AuthService.getJWTAccessToken();
		if (token) {
